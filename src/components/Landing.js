...
export default function Landing() {
  ...
  const filteredRecipes = recipes.filter((recipe) => {
    ...
    if (
      recipe.title.toLowerCase().includes(search.text.toLowerCase()) &&
      parseInt(radio) === 0
    )
      return true;
    else if (
      recipe.title.toLowerCase().includes(search.text.toLowerCase()) &&
      recipe.category_id === parseInt(radio)
    )
      return true;
    return false;
  });
  ...
  useEffect(() => {
    ...
    if (token) {
      axiosWithAuth()
        .get("https://tt75-recipes.herokuapp.com/api/categories")
        .then((res) => setCategories(res.data))
        .catch((err) => console.log(err));
      axiosWithAuth()
        .get("https://tt75-recipes.herokuapp.com/api/recipes")
  ...
  if (tokenExists) {
    return (
      <div className="landing">
        <div
          id="logo"
          style={{ height: "200px", width: "500px", margin: "auto" }}
        >
          <LogoLarge />
        </div>
